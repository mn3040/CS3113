...
void update()
{
    ...
    g_game_state.lander->update(delta_time, g_platform_positions, meteor_position);
...
void render() {
    ...
    // draw win message
    if (g_game_win == true) {
        glm::mat4 win_matrix = glm::mat4(1.0f);
        win_matrix = glm::translate(win_matrix, glm::vec3(0.0f, 0.0f, 0.0f));
        win_matrix = glm::scale(win_matrix, glm::vec3(0.25f, 0.25f, 0.0f));
        draw_object(win_matrix, g_win_message_texture_id);
    }
	// draw game over message
    if (g_game_over == true) {
        glm::mat4 game_over_matrix = glm::mat4(1.0f);
        game_over_matrix = glm::translate(game_over_matrix, glm::vec3(0.0f, 0.0f, 0.0f)); // Center of the screen
        game_over_matrix = glm::scale(game_over_matrix, glm::vec3(0.25f, 0.25f, 0.0f)); // Make it larger
        draw_object(game_over_matrix, g_gameOver_texture_id);
    }
