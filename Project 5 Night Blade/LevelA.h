...
private:
    ...
    const float ATTACK_DURATION = 0.5f; // duration of attack animation
	const float ATTACK_COOLDOWN = 1.0f; // cooldown before next attack
    ...
    const float STUCK_TIMEOUT = 1.5f; // 1.5 seconds of not moving = stuck
    ...
    // enemy animation state tracking (similar to player)
	...
	// helper functions to switch animations
